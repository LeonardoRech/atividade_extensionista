...
export default function EletronicTemplate(){
    ...
    useEffect(() => {
        axios.get(`${baseApiUrl}/eletronics`).then(resp => {
    ...
    function remove(eletronic) {
        axios.delete(`${baseApiUrl}/eletronics/${eletronic.id}`).then(res => {
            const list = getUpdatedList(eletronic, false)
            setList(list)
        })

        alert("Eletrônico Excluído da Lista!")
    }
            ...
            <div className='flex flex-col'>
                ...
                `}>
                    <thead className={`
                        ...
                        text-xl
                    `}>
                        <th className='p-2 font-light'>Verificar</th>
                        ...
                        {list.map((eletronic, i) => {
                            return (
                                <tr key={eletronic.id} className={`
                                    ...
                                    hover:bg-gray-900
                                `}>
                                    <a href={`/eletronics/${eletronic.id}`}><td className='px-2 py-5'>Verificar</td></a> 
